class SudokuPuzzle:
    ...
    def get_carre_valeurs(self, x, y):  # Ne pas effacer cette ligne
        ...
        """
        l = [(i, j) for i in range(self.dimension) for j in range(self.dimension)]
        numbers = []

        for X, Y in l:
            carre = self.get_carre(X, Y)
            numbers.append(carre.get_val(y, x))
        return numbers
    ...
    def get_ligne(self, ligne):  # Ne pas effacer cette ligne
        ...
        Y = ligne % n
        for i in range(n):
            carre = self.get_carre(i, X)
            for j in range(n):
                numbers.append(carre.get_val(j, Y))
        return numbers
    ...
    def get_colonne(self, colonne):  # Ne pas effacer cette ligne
        ...
        Y = colonne % n
        for i in range(n):
            carre = self.get_carre(X, i)
            for j in range(n):
                numbers.append(carre.get_val(Y, j))
        return numbers

    def correct_values(self, lst):
        N = self.dimension
        return sorted(lst) == [i + 1 for i in range(N * N)]

    def est_correct_lignes(self):
        N = self.dimension
        OK = True
        for ligne in range(N * N):
            OK = OK and self.correct_values(self.get_ligne(ligne))
        return OK

    def est_correct_colonnes(self):
        N = self.dimension
        OK = True
        for colonne in range(N * N):
            OK = OK and self.correct_values(self.get_colonne(colonne))
        return OK

    def est_correct_carres(self):
        N = self.dimension
        OK = True
        for x in range(N):
            for y in range(N):
                OK = OK and self.correct_values(self.get_carre_valeurs(x, y))
        return OK

    def est_correct(self):
        horizontal_ok = self.est_correct_lignes()
        vertical_ok = self.est_correct_colonnes()
        carres_ok = self.est_correct_carres()
        return (horizontal_ok and vertical_ok and carres_ok)
